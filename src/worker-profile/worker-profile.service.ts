...
export class WorkerProfileService {
  ...
  async getSkills(workerId: string) {
    ...
    if (error) throw new InternalServerErrorException('Error al obtener categorías');
    return data;
  }
  ...
  async setWorkerSkills(workerId: string, dto: SetWorkerSkillsDto) {
    ...
    if (uniqueCategoryIds.length !== categoryIds.length) {
      throw new BadRequestException('No puedes seleccionar categorías repetidas');
    }

    const { data: validCategories, error: categoriesError } = await this.supabaseService.client
      .from('service_categories')
      .select('id')
      .in('id', uniqueCategoryIds)
      .eq('is_active', true);

    if (categoriesError) throw new InternalServerErrorException('Error al validar categorías');

    const validIds = new Set((validCategories ?? []).map((category) => category.id));
    const invalidIds = uniqueCategoryIds.filter((categoryId) => !validIds.has(categoryId));

    if (invalidIds.length > 0) {
      throw new BadRequestException('Una o más categorías no pertenecen al catálogo oficial activo');
    }
    ...
    if (deleteError) throw new InternalServerErrorException('Error al actualizar categorías');
    ...
    if (error) throw new InternalServerErrorException('Error al guardar categorías');
    return data;
  }
  ...
  async uploadPortfolioFile(workerId: string, file: Express.Multer.File, title?: string) {
    await this.getVerifiedWorkerProfile(workerId);
    ...
    if (cleanTitle && cleanTitle.length > 80) {
      throw new BadRequestException('El título no puede superar los 80 caracteres');
    }
    ...
    if (upload.error) {
      throw new InternalServerErrorException(`Error al subir archivo de portafolio: ${upload.error.message}`);
    }
    ...
    if (error) throw new InternalServerErrorException('Error al guardar item del portafolio');
    return data;
  }
  ...
  private validatePortfolioFile(file: Express.Multer.File) {
    ...
    if (file.size > maxSizeInBytes) {
      throw new BadRequestException(`El archivo ${file.originalname} supera el tamaño máximo permitido de 20 MB`);
    }
  }
  ...
  private getPortfolioFileType(file: Express.Multer.File): 'image' | 'video' {
    ...
    if (file.mimetype.startsWith('video/') || fileName.endsWith('.mp4') || fileName.endsWith('.mov') || fileName.endsWith('.webm')) {
      return 'video';
    }
